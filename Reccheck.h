...
{
public:
	static uintptr_t unprotect(BYTE* funcaddr);
private:
	...
	static bool disable_retcheck(uintptr_t new_func, uintptr_t func_len)
	{
		bool has_retcheck = false;
		uintptr_t disasm_ptr = new_func;
		while (disasm_ptr - new_func < func_len)
		{
			...
			{
				uintptr_t ptr = disasm.disp.disp32;
	...
	static BYTE test_function(uintptr_t func)
	{
		__try
		{
			BYTE x = *(BYTE*)func;
	...
	static void fix_calls(uintptr_t new_func, uintptr_t orig_func, uintptr_t func_len)
	{
		uintptr_t disasm_ptr = new_func;
		while (disasm_ptr - new_func < func_len)
		{
			...
			{
				uintptr_t rel_addr = disasm.imm.imm32;
				uintptr_t orig_call_instr = orig_func + (disasm_ptr - new_func);
				uintptr_t orig_called_func = orig_call_instr + rel_addr + 5;
				if (orig_called_func % 0x10 == 0 && test_function(orig_called_func)) //functions are aligned to 0x10 bytes
				{
					__try
					{
						uintptr_t new_called_func = unprotect((BYTE*)orig_called_func);
						uintptr_t new_rel_addr = new_called_func - disasm_ptr - 5;
						*(uintptr_t*)(disasm_ptr + 1) = new_rel_addr;
					}
					__except (EXCEPTION_EXECUTE_HANDLER)
					{
						//ignore it
